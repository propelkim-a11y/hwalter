...
describe("SortableMainCard interaction", () => {
  ...
  function renderCard(onToggle = vi.fn(), onDragStateChange = vi.fn()) {
    render(
      <SortableMainCard cardId="tree" label="나의 나무" order={0} visible onMove={vi.fn()} onDragStateChange={onDragStateChange}>
        <button type="button" onClick={onToggle}>카드 열기</button>
      </SortableMainCard>,
    );
    return { onToggle, onDragStateChange, button: screen.getByRole("button", { name: "카드 열기" }) };
  }
  ...
  it("길게 누르는 동안에만 카드 이동 안내용 상태를 알린다", () => {
    const { button, onDragStateChange } = renderCard();
